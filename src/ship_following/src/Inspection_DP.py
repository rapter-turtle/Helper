...
class Control_Param():
    def __init__(self):
        ...
        self.filtered_data_ex = deque(maxlen=window_size)
        self.filtered_data_ey = deque(maxlen=window_size)
        self.ex_raw = 0.0
        self.ey_raw = 0.0
        ...
        nyquist_frequency = 0.5 * 10  
        order = 4  
        fc = 2.0  

        # Design the filter with the new cutoff frequency
        self.b, self.a = signal.butter(order, fc / nyquist_frequency, btype='low')
    ...
    def control_time(self,msg):
        ...
        ######################################################################## Axis calculation #########################################################################
                ...
                self.ey_raw = normal_distance(self.rel_x, self.rel_y, self.rel_yaw)
                self.ex_raw , self.threshold_length = bound_length(self.rel_x, self.rel_y, self.rel_yaw, math.pi*30/180, self.ds)


                ######################################### Low Pass Filter ######################################### 
                if self.zi_y is None:
                    self.zi_y = signal.lfiltic(self.b, self.a, [self.ey_raw])
                if self.zi_x is None:
                    self.zi_x = signal.lfiltic(self.b, self.a, [self.ex_raw])


                filtered_data_point_ey, self.zi_y = signal.lfilter(self.b, self.a, [self.ey_raw], zi=self.zi_y)
                self.filtered_data_ey.append(filtered_data_point_ey[0])

                filtered_data_point_ex, self.zi_x = signal.lfilter(self.b, self.a, [self.ex_raw], zi=self.zi_x)
                self.filtered_data_ex.append(filtered_data_point_ex[0])

                ###################################################################################################
                self.e_y = filtered_data_point_ey[0]
                self.e_x = filtered_data_point_ex[0]
