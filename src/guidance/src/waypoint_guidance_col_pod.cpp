  ...
  private:
	...
	void DM()
	{
		...
		if(bApp){                           // Approach
			...
			else if (Dist <0.5*R_s && Dist > 0.4*R_s)
			{
				std::vector<double> theta_out = {2*cos(theta_out_center),2*sin(theta_out_center)};
				std::vector<double> theta_wp = {1*cos(psi_wp), 1*sin(psi_wp)};
	 ...
	 // lla2utm(lat_0, lon_0, 0);
	  ...
	  if(wp_call)
	  {
		  ...
		  if (stage == 3)
		  {
			  ...
			  if(thrust_left_pod_msg.data>90){
				thrust_left_pod_msg.data = 90;
			  }
			  if(thrust_left_pod_msg.data<-90){
				thrust_left_pod_msg.data = -90;
			  }
			  if(thrust_right_pod_msg.data>90){
				thrust_right_pod_msg.data = 90;
			  }
			  if(thrust_right_pod_msg.data<-90){
				thrust_right_pod_msg.data = -90;
			  }
			  ...
			  if (yaw > 0.001 || yaw < -0.001){
				...
				thrust_left_pod_msg.data = 0.0;
				thrust_right_pod_msg.data = 0.0;
				pub_thrust_left.publish(thrust_left_msg);
				pub_thrust_right.publish(thrust_right_msg);
				pub_thrust_left_pod.publish(thrust_left_pod_msg);
				pub_thrust_right_pod.publish(thrust_right_pod_msg);
	...
	double psi_wp;    
	double psi_d;
	double V_norm = 3.8; // Approximately assume
	double R_s = 40;
