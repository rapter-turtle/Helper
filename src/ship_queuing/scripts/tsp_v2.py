...
class WPRouting():
    ...
    def wp_callback(self, msg):
        ...
        if self.flag :
            ...
            wp_size = int(msg.data[4])+1
            for i in range(1,wp_size):
                for j in self.visited_WPS_ids_list:
                    if j == origin_wps[5*i]:
                        break
                else:    
                    wps_ids_xy.append((origin_wps[5*i],origin_wps[5*i+1],origin_wps[5*i+2]))
                    wps_xy.append((origin_wps[5*i+1],origin_wps[5*i+2]))
            ...
            msg_pub_1.data = wps_ids_xy[self.WPS_this_order][0]
            msg_pub_2.data = 1

            self.owppub.publish(msg_pub_1)
            self.flagpub.publish(msg_pub_2)
            
            self.visited_WPS_ids_list.append(wps_ids_xy[self.WPS_this_order])
            self.flag = 0
